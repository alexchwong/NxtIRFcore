...
#'   `reference_path`. If `GetReferenceResource()` has already been run, it will
#'   store a copy of the genome and this parameter can be omitted.
...
#' @return None. `Mappability_GenReads()` writes `Reads.fa` to the `Mappability`
#'   subdirectory inside the given `reference_path`.
#'   `Mappability_CalculateExclusions()` writes a gzipped BED file
#'   named `MappabilityExclusion.bed.gz` to the `Mappability` subdirectory
#'   inside `reference_path`. 
#'   This BED file can be used as input as `MappabilityRef` in 
#'   `BuildReference()`
#' @examples
#' 
...
#' )
#'
#' # (4) Load the Mappability Exclusion BED file as a GRanges object
#'
#' MapExc.gr = rtracklayer::import(
#'     con = file.path(ref_path, "Mappability", "MappabilityExclusion.bed.gz"),
#'     format = "bed" 
#' )
#' }
