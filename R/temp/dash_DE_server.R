server_DE <- function(id, refresh_tab, volumes, get_threads,
        get_se, get_filters,
        get_selected_diag, get_selected_volc) {
    ...
    observeEvent(input$variable_DE, {
        ...
        updateSelectInput(session = session, inputId = "variable_DE", 
            ...
            if(is_valid(settings_DE$nom_DE) && 
                    settings_DE$nom_DE %in% 
                        levels(colData[,input$variable_DE])) {
                updateSelectInput(session = session, inputId = "nom_DE", 
                    selected = settings_DE$nom_DE)
            }
            if(is_valid(settings_DE$nom_DE) && 
                    settings_DE$denom_DE %in% 
                        levels(colData[,input$variable_DE])) {
                updateSelectInput(session = session, inputId = "denom_DE", 
    ...
    observeEvent(settings_DE$command_selected, {
        if(is_valid(settings_DE$command_selected) && !identical(
            settings_DE$command_selected, input$DT_DE_rows_selected
        )) {
