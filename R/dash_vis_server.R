server_vis_diag <- function(id, refresh_tab, volumes, get_se, get_de,
    ...
    moduleServer(id, function(input, output, session) {
        ...
        observeEvent(refresh_tab(), {
            ...
            if(is_valid(input$variable_diag) && 
    ...
    output$plot_diag <- renderPlotly({
        ...
        df.diag = make_diagonal(get_se(), res$EventName, input$variable_diag,
        input$nom_diag, input$denom_diag)

        if(is_valid(settings_Diag$selected)) {
            df.diag$selected = (df.diag$EventName %in% 
                get_de()$EventName[selected])
        } else {
        ...
        if(input$NMD_diag == TRUE) {
            ...
            df.diag$denom_NMD = ifelse(df.diag$NMD_direction == -1, 
                df.diag$nom, df.diag$denom)
            p = ggplot(df.diag, 
                    aes(x = get("nom_NMD"), y = get("denom_NMD"), 
                        key = get("EventName"), 
                        text = get("EventName"), colour = get("selected"))
                ) + geom_point() + 
            ...
            p = ggplot(df.diag, aes(x = get("nom"), y = get("denom"), 
                    key = get("EventName"), 
                    text = get("EventName"), colour = get("selected"))
                ) + geom_point() + 
                scale_color_manual(values = c("black", "red")) +
...
server_vis_volcano <- function(id, refresh_tab, volumes, get_se, get_de,
        rows_all, rows_selected) {
    moduleServer(id, function(input, output, session) {
    ...
    output$plot_volc <- renderPlotly({
        ...
        if(is_valid(selected)) {
            df.volc$selected = (df.volc$EventName %in% 
                get_de()$EventName[selected])
        } else {
        ...
        if(input$NMD_volc == TRUE) {
            ...
            df.volc$log2FoldChange = df.volc$log2FoldChange * 
                df.volc$NMD_direction
        }
...
server_vis_heatmap <- function(id, refresh_tab, volumes, get_se, get_de,
        rows_all, rows_selected) {
    moduleServer(id, function(input, output, session) {
