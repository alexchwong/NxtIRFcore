...
CoverageBlock::CoverageBlock(unsigned int start, unsigned int end) {
	blockStart = start;
	blockEnd = end;
...
void CoverageBlock::updateCoverageHist(std::map<unsigned int,unsigned int> &hist, unsigned int start, unsigned int end, bool dir) const {
	if (!blockExtentsL && !blockExtents) {
		// how many bases in this block?
