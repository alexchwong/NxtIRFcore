---
title: 'NxtIRFcore: Interactive Differential Alternative Splicing and Intron Retention analysis'
...
output:
    rmarkdown::html_document:
        toc: true
vignette: >
    %\VignetteIndexEntry{NxtIRFcore: Interactive Differential Alternative Splicing and Intron Retention analysis}
...
library(NxtIRFcore)
NxtIRFdata::chrZ_genome() # Provides the path to the NxtIRF chrZ example genome
NxtIRFdata::chrZ_gtf()    # Provides the path to the NxtIRF chrZ example GTF
```
...
```{r, results = FALSE, message = FALSE, warning = FALSE}
bams = Find_Bams("./bams", level = 0) # as BAM file names denote their sample names

# In the case where BAM files are labelled using sample names as parent directory names, use:
# Find_Bams("./bams", level = 1)
```
